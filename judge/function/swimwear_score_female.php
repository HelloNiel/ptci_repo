    ...
    if ($judge_id !== null) {
        foreach ($_POST['score'] as $cand_no => $score) {
            if (!empty($score) && is_numeric($score) && $score >= 7 && $score <= 10) {
                $stmt = $conn->prepare("INSERT INTO swimwear_score_female (cand_no, score, judge_id) 
                                        VALUES (?, ?, ?)
        ...
        $total_stmt = $conn->prepare("
            INSERT INTO female_candidate_total_scores (cand_no, swim_total)
            SELECT cand_no, AVG(score) 
            FROM swimwear_score_female
            GROUP BY cand_no
            ON DUPLICATE KEY UPDATE swim_total = VALUES(swim_total);
        ");
...
$conn->close();
header("Location: ../femaleswimwear.php");
exit();
?>
