    ...
    if ($judge_id !== null) {
        foreach ($_POST['score'] as $cand_no => $score) {
            if (!empty($score) && is_numeric($score) && $score >= 7 && $score <= 10) {
                $stmt = $conn->prepare("INSERT INTO gown_score_female (cand_no, score, judge_id) 
                                        VALUES (?, ?, ?)
        ...
        $total_stmt = $conn->prepare("
            INSERT INTO female_candidate_total_scores (cand_no, gown_total)
            SELECT cand_no, AVG(score) 
            FROM gown_score_female
            GROUP BY cand_no
            ON DUPLICATE KEY UPDATE gown_total = VALUES (gown_total);
        ");
...
$conn->close();
header("Location: ../female_gown.php");
exit();
?>
