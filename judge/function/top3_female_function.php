    ...
    if ($judge_id !== null) {
        foreach ($_POST['score'] as $cand_no => $score) {
            if (!empty($score) && is_numeric($score) && $score >= 7 && $score <= 10) {
                $stmt = $conn->prepare("INSERT INTO top3_scores_female (cand_no, score, judge_id, cand_fn, cand_ln, cand_team) 
                                        VALUES (?, ?, ?, ?, ?, ?)
                                        ON DUPLICATE KEY UPDATE score = VALUES(score)");
                if ($stmt) {
                    $candidate_stmt = $conn->prepare("SELECT cand_fn, cand_ln, cand_team FROM top3_candidate_female WHERE cand_no = ?");
